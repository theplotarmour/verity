...
| 111_global_settings_and_shell_ux_pattern.md | **NEW 2026-09-17, open question RESOLVED same day** — codifies Settings/Appearance shell + Home dashboard IA as the global shell/settings pattern for all tenants (structure; palette stays ADR-012). Its flagged ADR-weight question (solid cards: forms exemption only, or platform-wide?) resolved by **ADR-023** — product owner explicitly retired glass as the default material platform-wide. Nothing built yet; buildable now |
| 112_complete_ui_ux_upgrade_to_structured_minimalism.md | **NEW 2026-09-17** — platform-wide rollout of ADR-023/Task 111's solid material to every tab/page/capability/tenant (Task 111 itself scoped to Settings+Home shell only). Nothing migrated yet; `globals.css` still defines the deprecated glass classes, Outreach's dashboard still renders glass. Buildable now, land incrementally per capability |
| 113_ai_implementation_audit_all_clients_and_global_agent.md | **NEW 2026-09-17** — audit (not build): is Task 84's global AI agent actually working per real tenant, is Task 106 Phase 8's Groq-no-tool-calling block still current, what of Task 95's AI vision is BUILT vs. still aspirational. No prior session has re-checked Task 106's BLOCKED status since 2026-09-14/15 |
