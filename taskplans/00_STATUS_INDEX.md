...
| 117_colonel_kebabz_ui_completion.md | **IN PROGRESS 2026-09-20.** Odoo-lens audit found Colonel Kebabz capabilities registering nav that 404'd (or, for `crm`, collided with plywood's `/customers`) and `coupon` with zero nav/query at all. Phases 0-7 all have a working screen now (`/guests`, `/complaints`, `/attendance`, `/coupons`, `/expenses`, `/recipes`), `tsc` clean; payroll-inputs report, shift admin, cash-reconciliation/outlet-P&L routes, counter coupon-apply, and recipe/BOM authoring on `/menu` remain — see the taskplan's own Status section |
