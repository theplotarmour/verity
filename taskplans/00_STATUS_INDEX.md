...
| 117_colonel_kebabz_ui_completion.md | **IN PROGRESS 2026-09-20.** Odoo-lens audit found 7 Colonel Kebabz capabilities (recipe/crm/loyalty/coupon/complaint/attendance/finance) shipped backend-only per `clients/colonel-kebabz/phase-plan.md`, no UI route, plus a real bug (`crm`'s nav contribution targets `/customers`, which plywood's page actually serves). 8-phase build in progress |
