...
| 117_colonel_kebabz_ui_completion.md | **BUILT 2026-09-20.** Odoo-lens audit found Colonel Kebabz capabilities registering nav that 404'd (or, for `crm`, collided with plywood's `/customers`) and `coupon` with zero nav/query at all. All 8 phases now have a working screen (`/guests`, `/complaints`, `/attendance` incl. payroll/shifts, `/coupons`, `/expenses`, `/cash-reconciliation`, `/outlet-pnl`, `/recipes` incl. per-item BOM authoring), counter coupon redemption wired, `tsc` clean throughout. Real logged-in verification pass still open — no live DB in this environment |
