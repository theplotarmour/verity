...
> **GAP — `inputs` unanswered for this action**  
> Not specified in the model. This is a task, not an omission to be papered over. Owner: TBD. Blocks: implementation of `core_identity_session.logout.inputs`.

### What is created

> **GAP — `creates` unanswered for this action**  
> Not specified in the model. This is a task, not an omission to be papered over. Owner: TBD. Blocks: implementation of `core_identity_session.logout.creates`.
...
> **GAP — `notifications` unanswered for this action**  
> Not specified in the model. This is a task, not an omission to be papered over. Owner: TBD. Blocks: implementation of `core_identity_session.logout.notifications`.
...
> **GAP — `concurrency` unanswered for this action**  
> Not specified in the model. This is a task, not an omission to be papered over. Owner: TBD. Blocks: implementation of `core_identity_session.logout.concurrency`.
...
**EC-03.** KDS LOGOUT: a KDS screen bound to a site does not truly log out to an anonymous state — it returns to a station picker while retaining the device-level station credential. A human logging out of a KDS must not orphan in-flight kitchen tickets; open tickets are already server-side, so this is safe, but the client must confirm 'N tickets in progress remain on the server' rather than showing an empty screen.
