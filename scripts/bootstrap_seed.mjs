...
const hashPin = (pin, factoryId) => createHash('sha256').update(`verity:${factoryId}:${pin}`).digest('hex')

// ---- Factory ----
let factory = await p.factory.findFirst({ where: { slug: 'carxen' } })
if (!factory) factory = await p.factory.create({ data: { name: 'Carxen', slug: 'carxen', onboardingStatus: 'COMPLETED' } })
else factory = await p.factory.update({ where: { id: factory.id }, data: { onboardingStatus: 'COMPLETED' } })
const factoryId = factory.id
console.log('factory', factoryId)

// ---- Owner credentials: phone 9971907190, PIN 7190 ----
const ownerPhone = '9971907190'
const pinHash = hashPin('7190', factoryId)
let owner = await p.user.findUnique({ where: { phone: ownerPhone } })
if (!owner) owner = await p.user.create({ data: { factoryId, name: 'Carxen Owner', phone: ownerPhone, role: 'OWNER', pinHash, isActive: true, status: 'active' } })
else owner = await p.user.update({ where: { id: owner.id }, data: { factoryId, role: 'OWNER', pinHash, isActive: true, status: 'active' } })
console.log('owner', owner.phone, 'PIN 7190')

// ---- Departments (production chain) ----
...
for (const d of DEFAULT_DEPARTMENTS) {
  const ex = await p.department.findFirst({ where: { factoryId, name: d.name } })
  if (!ex) await p.department.create({ data: { factoryId, ...d, active: true } })
}

// ---- Default QC/checklist template ----
let tpl = await p.checklistTemplate.findFirst({ where: { factoryId, status: 'active' } })
if (!tpl) {
  tpl = await p.checklistTemplate.create({ data: { factoryId, name: 'Seat Cover Quality Checks', status: 'active' } })
  const sec = await p.templateSection.create({ data: { factoryId, templateId: tpl.id, title: 'Final Inspection', sortOrder: 1 } })
  await p.checkpoint.createMany({ data: [
    { factoryId, sectionId: sec.id, name: 'Stitch alignment', instructions: 'Seams straight, 4mm spacing', requireImage: true, requireRemarks: false, sortOrder: 1 },
    { factoryId, sectionId: sec.id, name: 'Fitment', instructions: 'Cover sits flush, no gaps', requireImage: true, requireRemarks: false, sortOrder: 2 },
    { factoryId, sectionId: sec.id, name: 'Finish & cleanliness', instructions: 'No loose threads or marks', requireImage: false, requireRemarks: true, sortOrder: 3 },
  ] })
}
