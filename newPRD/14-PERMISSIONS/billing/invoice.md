---
doc_id: PERM-INVOICE
title: Permission matrix — Invoice
generated: true
...
> Not specified in the model. This is a task, not an omission to be papered over. Owner: TBD. Blocks: implementation of `invoice.permission_defaults`.
...
| `tax_total_minor` | `view_financial` | nullable rather than zero, so unknown tax and no tax never render identically |
| `total_minor` | `view_financial` | marked in model |
| `allocated_minor` | `view_financial` | marked in model |
| `disputed_minor` | `view_financial` | marked in model |
| `written_off_minor` | `view_financial` | marked in model |
