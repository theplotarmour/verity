---
doc_id: PERM-INVOICE_LINE
title: Permission matrix — Invoice Line
generated: true
...
# Permission matrix — Invoice Line
...
> Not specified in the model. This is a task, not an omission to be papered over. Owner: TBD. Blocks: implementation of `invoice_line.permission_defaults`.

## Field-level gates

| Field | Gate verb | Rationale |
|---|---|---|
| `unit_amount_minor` | `view_financial` | marked in model |
| `line_total_minor` | `view_financial` | marked in model |
| `tax_classification` | `view_financial` | marked in model |
| `tax_amount_minor` | `view_financial` | marked in model |
| `rate_rule_ref` | `view_financial` | marked in model |
