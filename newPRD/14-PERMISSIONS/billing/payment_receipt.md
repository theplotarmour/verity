---
doc_id: PERM-PAYMENT_RECEIPT
title: Permission matrix — Payment Receipt
generated: true
...
# Permission matrix — Payment Receipt
...
> Not specified in the model. This is a task, not an omission to be papered over. Owner: TBD. Blocks: implementation of `payment_receipt.permission_defaults`.
...
| `method` | `view_financial` | marked in model |
| `external_reference` | `view_financial` | the bank or provider reference. Unique per tenant per method where present, which is the control that catches a receipt recorded twice |
| `allocated_minor` | `view_financial` | marked in model |
| `reversed_reason` | `view_financial` | marked in model |
