---
doc_id: PERM-TICKET
title: Permission matrix — Ticket
generated: true
...
> Not specified in the model. This is a task, not an omission to be papered over. Owner: TBD. Blocks: implementation of `ticket.permission_defaults`.

## Field-level gates

| Field | Gate verb | Rationale |
|---|---|---|
| `reporter_contact_raw` | `view_sensitive` | the address or number the report arrived from, kept verbatim even where a party was matched, because the match may be wrong and the reply has to go back where it came from |
