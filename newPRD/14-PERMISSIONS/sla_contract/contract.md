---
doc_id: PERM-CONTRACT
title: Permission matrix — Contract
generated: true
...
> Not specified in the model. This is a task, not an omission to be papered over. Owner: TBD. Blocks: implementation of `contract.permission_defaults`.

## Field-level gates

| Field | Gate verb | Rationale |
|---|---|---|
| `billing_basis` | `view_financial` | marked in model |
| `currency` | `view_financial` | marked in model |
| `value_minor` | `view_financial` | marked in model |
| `penalty_cap_minor` | `view_financial` | the ceiling on penalties in a measurement period. Mandatory in practice and nullable in the model, because a contract genuinely without a cap exists and pretending otherwise would force a fictional number |
