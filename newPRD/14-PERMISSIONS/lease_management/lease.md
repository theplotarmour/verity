...
> Not specified in the model. This is a task, not an omission to be papered over. Owner: TBD. Blocks: implementation of `lease.permission_defaults`.

## Field-level gates

| Field | Gate verb | Rationale |
|---|---|---|
| `base_amount_minor` | `view_financial` | marked in model |
| `currency` | `view_financial` | marked in model |
| `payment_frequency` | `view_financial` | marked in model |
| `payment_in_advance` | `view_financial` | whether a period is paid at its start or its end. Explicit because it moves every due date by a whole period and is a frequent source of disagreement |
| `deposit_amount_minor` | `view_financial` | marked in model |
