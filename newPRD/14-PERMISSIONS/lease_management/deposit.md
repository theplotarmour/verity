---
doc_id: PERM-DEPOSIT
title: Permission matrix — Deposit
generated: true
...
> Not specified in the model. This is a task, not an omission to be papered over. Owner: TBD. Blocks: implementation of `deposit.permission_defaults`.

## Field-level gates

| Field | Gate verb | Rationale |
|---|---|---|
| `required_amount_minor` | `view_financial` | marked in model |
| `held_amount_minor` | `view_financial` | marked in model |
| `scheme_reference` | `view_sensitive` | marked in model |
| `returned_amount_minor` | `view_financial` | marked in model |
| `applied_amount_minor` | `view_financial` | applied against arrears or reinstatement, only by an explicit act |
| `application_reason` | `view_financial` | marked in model |
