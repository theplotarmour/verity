---
doc_id: PERM-BOOKING
title: Permission matrix — Booking
generated: true
...
> Not specified in the model. This is a task, not an omission to be papered over. Owner: TBD. Blocks: implementation of `booking.permission_defaults`.

## Field-level gates

| Field | Gate verb | Rationale |
|---|---|---|
| `deposit_required_minor` | `view_financial` | marked in model |
| `deposit_paid_minor` | `view_financial` | marked in model |
| `deposit_reference` | `view_financial` | the payment reference from the integrations capability. This capability never holds a payment credential |
| `access_requirements` | `view_sensitive` | anything the person needs in order to attend. Sensitive by default because it frequently discloses a health or personal circumstance, and it must still reach the person delivering the service |
