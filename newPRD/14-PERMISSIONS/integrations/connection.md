---
doc_id: PERM-CONNECTION
title: Permission matrix — Connection
generated: true
...
> Not specified in the model. This is a task, not an omission to be papered over. Owner: TBD. Blocks: implementation of `connection.permission_defaults`.

## Field-level gates

| Field | Gate verb | Rationale |
|---|---|---|
| `credential_ref` | `view_sensitive` | a reference into the secret store, never the secret. This capability holds references and the secret store holds secrets, and nothing in Verity ever returns one |
| `endpoint_url` | `view_sensitive` | marked in model |
